...
import com.doable.model.EmployeeTask;
import java.sql.*;
...
public class EmployeeTaskDao {
    ...
    public EmployeeTask save(EmployeeTask task) {
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "INSERT INTO employee_tasks (title, description, due, completed, repeat_rule, category_id, created_by_employee_id, created_date, due_date, marked_for_completion) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ...
            ps.setInt(7, task.getCreatedByEmployeeId());
    ...
    public EmployeeTask findById(long id) {
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "SELECT t.*, c.name FROM employee_tasks t " +
    ...
    public List<EmployeeTask> findByEmployeeId(int employeeId) {
        List<EmployeeTask> list = new ArrayList<>();
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "SELECT t.*, c.name FROM employee_tasks t " +
                     "LEFT JOIN categories c ON t.category_id = c.id " +
                     "WHERE t.created_by_employee_id = ? " +
                     "ORDER BY t.created_date DESC")) {
    ...
    public List<EmployeeTask> findByEmployeeIdAndCompleted(int employeeId, boolean completed) {
        List<EmployeeTask> list = new ArrayList<>();
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "SELECT t.*, c.name FROM employee_tasks t " +
                     "LEFT JOIN categories c ON t.category_id = c.id " +
                     "WHERE t.created_by_employee_id = ? AND t.completed = ? " +
                     "ORDER BY t.created_date DESC")) {
            ps.setInt(1, employeeId);
    ...
    public void update(EmployeeTask task) {
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "UPDATE employee_tasks SET title = ?, description = ?, due = ?, completed = ?, " +
    ...
    public void delete(long id) {
        try (Connection conn = db.getConnection();
             PreparedStatement ps = conn.prepareStatement("DELETE FROM employee_tasks WHERE id = ?")) {
            ps.setLong(1, id);
            ps.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    private EmployeeTask mapRow(ResultSet rs) throws SQLException {
